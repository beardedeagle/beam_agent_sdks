%%%-------------------------------------------------------------------
%%% @doc Pure SSE (Server-Sent Events) frame parser.
%%%
...
%% @doc Create a fresh parse state.
...
%% @doc Return the current internal buffer size in bytes.
...
%% @doc Parse a new chunk of SSE data with the current parse state.
%%
...
%% @doc Process a list of complete lines, accumulating event state.
...
%% @doc Parse a single field line and update the event accumulator.
...
%% @doc Flush the accumulated event state into an sse_event() map.
